...
exports.computeTokenColors = computeTokenColors;
...
// ============================================
function computeMultipliers(settings) {
...
function computeColors(settings) {
    const p = palettes_1.palettes[settings.preset] || palettes_1.palettes.dark;
    const m = computeMultipliers(settings);
    const bg = (hex) => adjust(hex, { brightMul: m.brightMul, warmShift: m.warmShift });
    const fg = (hex) => adjust(hex, { contrastMul: m.contrastMul, warmShift: m.warmShift });
    const syn = (hex) => adjust(hex, { satMul: m.satMul, contrastMul: m.contrastMul, warmShift: m.warmShift });
...
// ============================================
function computeTokenColors(settings) {
    const p = palettes_1.palettes[settings.preset] || palettes_1.palettes.dark;
    const m = computeMultipliers(settings);
    const syn = (hex) => adjust(hex, { satMul: m.satMul, contrastMul: m.contrastMul, warmShift: m.warmShift });
    return {
