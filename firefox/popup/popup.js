/**
 * KeratoVision Popup Script v2.3 — Firefox Edition
 *
 * Uses browser.* Promise-based API (MV2).
 * Key difference: browser.tabs.executeScript instead of chrome.scripting.executeScript
 */
...
(function () {
    ...
    // === Content Script Injection (Firefox MV2) ===
    async function ensureContentScripts(tabId) {
        try {
            const response = await browser.tabs.sendMessage(tabId, { type: 'GET_STATUS' }).catch(() => null);
            if (response && response.version) return true;

            // Firefox MV2: inject scripts one by one
    ...
    function pushUpdate() {
        ...
        saveTimer = setTimeout(() => browser.storage.sync.set({ ...currentConfig }), 50);
    }
    ...
    browser.storage.onChanged.addListener((changes, area) => {
        if (area !== 'sync') return;
        browser.storage.sync.get(DEFAULT_CONFIG).then((config) => populateUI(config));
    });
